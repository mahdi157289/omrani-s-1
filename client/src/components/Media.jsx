        ...
        <div className="bg-white/40 backdrop-blur-xl border border-white/40 rounded-[3rem] p-12 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-16 relative z-10">
            <div className="flex items-center gap-4 mb-3">
               ...
               <span className="font-medium tracking-widest uppercase text-sm text-amber-900/80">{t('media.subtitle')}</span>
               <span className="h-px w-12 bg-amber-900 opacity-30"></span>
            </div>
            ...
            <p className="text-amber-900/70 mt-8 max-w-2xl mx-auto text-center font-medium">{t('media.description')}</p>
          </div>
              ...
              {videos.length > 0 ? (
                videos.slice(1).map((video, index) => (
                  <motion.div 
                    ...
                    className="bg-white/60 backdrop-blur-sm border border-white/40 rounded-2xl overflow-hidden shadow-lg group cursor-pointer hover:shadow-xl transition-all"
                  ...
                  >
                    <div className="flex items-center gap-4 p-4">
                      ...
                      <div>
                        <h4 className="font-serif font-bold text-amber-950 line-clamp-1">{video.title}</h4>
                        <p className="text-amber-900/60 text-sm mt-1 line-clamp-2">{video.description}</p>
                      </div>
                    </div>
                  </motion.div>
                ))
                ...
                [
                  ...
                  >
                    <div className="flex items-center gap-4 p-4">
                      ...
                      <div>
                        <h4 className="font-serif font-bold text-amber-950">{video.title}</h4>
                        <p className="text-amber-900/60 text-sm mt-1">{video.time}</p>
                      </div>
                    </div>
                  </motion.div>
                ))
          ...
          <div className="grid md:grid-cols-4 gap-6 mt-12 relative z-10">
            {[
              ...
              <motion.div 
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="bg-white/60 backdrop-blur-sm border border-white/40 rounded-2xl p-6 text-center shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 transform duration-300"
              >
                <span className="text-4xl block mb-3">{item.icon}</span>
                <h4 className="font-serif font-bold text-amber-950">{item.title}</h4>
                <p className="text-amber-900/60 text-sm mt-2 font-medium">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
